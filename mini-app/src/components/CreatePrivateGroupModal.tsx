...
import { UserProfile, WinningPattern, PrizeDistributionRule } from '@shared/types';
...
export const CreatePrivateGroupModal: React.FC<CreatePrivateGroupModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-end sm:items-center justify-center p-2.5 sm:p-4 animate-in fade-in pb-safe">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-4 sm:p-6 space-y-4 sm:space-y-5 shadow-2xl max-h-[88vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-3 py-2.5 text-xs text-white focus:outline-none focus:border-amber-500 font-bold"
              >
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                ...
                <div className="text-[10px]">All 4 corner numbers only</div>
              </button>

              <button
                type="button"
                onClick={() => setWinningPattern('ONE_LINE_AND_CORNERS')}
                className={`p-2.5 rounded-2xl border text-left transition col-span-2 ${
          ...
          <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-950 border border-slate-800">
            <div>
              <div className="text-xs font-bold text-white">Auto-Start when Players Ready</div>
              <div className="text-[10px] text-slate-400">
                Game starts automatically when all participants buy tickets
              </div>
            </div>
            <input
              type="checkbox"
