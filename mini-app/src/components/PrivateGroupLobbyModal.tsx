...
import { UserProfile, PrivateGroup, GroupMember, GroupMessage, BingoRoom } from '@shared/types';
import { CardSelectionView } from './CardSelectionView';
import { formatCardNumber } from '@shared/bingoUtils';
...
export const PrivateGroupLobbyModal: React.FC<PrivateGroupLobbyModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in overflow-y-auto">
      <div className="bg-slate-900 border border-slate-800 rounded-t-3xl sm:rounded-3xl w-full max-w-lg mx-auto sm:my-8 my-2 px-3 sm:px-6 py-4 sm:py-6 space-y-4 shadow-2xl max-h-[92vh] flex flex-col justify-between overflow-y-auto">
        {/* Header Bar */}
