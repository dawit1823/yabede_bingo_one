...
import { BingoRoom, UserProfile, CardReservation, BingoTicket, ChatMessage } from '../types';
...
export const CardSelectionView: React.FC<CardSelectionViewProps> = ({
  ...
  // Group Chat States for Card Selection Screen
  const [chatOpen, setChatOpen] = useState<boolean>(false);
  const [messageInput, setMessageInput] = useState<string>('');
  const messagesEndRef = useRef<HTMLDivElement>(null);

  // Filter messages strictly for current room to guarantee room-specific chat isolation
  ...
  return (
    <div className="space-y-4 transition-all duration-300 relative pb-8">
      ...
      <div className="flex items-center justify-between bg-slate-900 border border-slate-800 rounded-3xl p-4 shadow-xl">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            ...
            className="relative px-2.5 sm:px-3 py-2 rounded-2xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30 text-xs font-bold transition flex items-center gap-1.5 active:scale-95 cursor-pointer"
          >
            <MessageSquare className="w-3.5 h-3.5 text-amber-400" />
            <span>{language === 'am' ? 'ቻት' : 'Chat'}</span>
            {roomMessages.length > 0 && (
              <span className="absolute -top-1 -right-1 px-1.5 py-0.2 bg-amber-400 text-slate-950 font-black text-[9px] rounded-full">
                {roomMessages.length}
              </span>
            )}
          </button>
          ...
          <button
            ...
            className="px-2.5 sm:px-3 py-2 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 text-xs font-bold transition flex items-center gap-1.5 active:scale-95 disabled:opacity-50 cursor-pointer"
          >
            <RefreshCw className={`w-3.5 h-3.5 text-slate-300 ${isRefreshing ? 'animate-spin' : ''}`} />
            <span className="hidden sm:inline">{language === 'am' ? 'አድስ' : 'Refresh'}</span>
          </button>
        </div>
      </div>

      {/* Quick Enter Game Header Bar if cards are selected */}
      {mySelectedCards.length > 0 && (
        <div className="bg-emerald-950/90 border-2 border-emerald-500/60 rounded-2xl p-3 px-4 flex items-center justify-between shadow-lg animate-fadeIn">
          ...
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs transition flex items-center gap-1.5 active:scale-95 cursor-pointer shadow-md"
          >
            <span>{language === 'am' ? 'ወደ ጨዋታው ግባ' : 'Enter Arena'}</span>
            <ArrowLeft className="w-3.5 h-3.5 rotate-180" />
          </button>
        </div>
      )}
      ...
      {/* Floating Chat Trigger for easy access while browsing 400 cards */}
      {!chatOpen && (
        <button
          onClick={() => {
            setChatOpen(true);
          ...
          className="fixed bottom-24 right-4 z-40 bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 px-3.5 py-2.5 rounded-full font-black text-xs shadow-2xl flex items-center gap-2 border-2 border-amber-300 hover:scale-105 active:scale-95 transition cursor-pointer"
        >
          <MessageSquare className="w-4 h-4" />
          <span>{language === 'am' ? 'የክፍል ቻት' : 'Room Chat'}</span>
          {roomMessages.length > 0 && (
      ...
      {/* Room Group Chat Drawer */}
      {chatOpen && (
        <div className="fixed inset-x-0 bottom-0 z-50 bg-slate-900/95 backdrop-blur-md border-t border-slate-800 p-4 rounded-t-3xl shadow-2xl max-h-[85vh] h-[380px] flex flex-col justify-between space-y-3 animate-slideUp">
          <div className="flex items-center justify-between border-b border-slate-800 pb-2">
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <h4 className="text-xs font-black text-slate-100 flex items-center gap-1.5">
                <span>{liveRoom.name || 'Room'}</span>
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-0.5">
            {[
              ...
              language === 'am' ? '🎟️ ካርድ መርጫለሁ!' : '🎟️ Cards selected!',
              language === 'am' ? '🍀 መልካም ዕድል!' : '🍀 Good luck!',
              language === 'am' ? '⚡ ጨዋታው ይጀምር!' : '⚡ Ready to play!',
            ].map((phrase, idx) => (
              <button
                key={idx}
                onClick={() => {
                  if (onSendMessage) onSendMessage(phrase);
                  triggerHaptic('light');
                }}
          ...
          {/* Chat Messages List */}
          <div className="flex-1 overflow-y-auto space-y-2 pr-1 max-h-52 min-h-[140px]">
            {roomMessages.length === 0 ? (
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' && messageInput.trim()) {
                  if (onSendMessage) onSendMessage(messageInput);
                  setMessageInput('');
                  triggerHaptic('light');
                }
              }}
