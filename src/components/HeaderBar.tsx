        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-amber-500 via-amber-400 to-yellow-300 p-0.5 shadow-lg shadow-amber-500/20 flex items-center justify-center">
            <div className="w-full h-full bg-slate-950 rounded-[10px] flex items-center justify-center">
              <span className="text-xl font-black text-amber-400 tracking-wider">ያ</span>
            </div>
          </div>
          <div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="relative" ref={themeRef}>
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border text-xs font-bold transition shadow-sm ${currentThemeObj.bgClass}`}
              title="Select Theme Palette"
            >
              {theme === 'golden' ? (
                <div className="flex items-center gap-1">
                  <Sunset className="w-4 h-4 text-amber-400 animate-pulse" />
                  <span className="hidden sm:inline text-amber-300 font-black tracking-wide">Golden Hour</span>
                </div>
              ) : theme === 'light' ? (
                <div className="flex items-center gap-1">
                  <Sun className="w-4 h-4 text-amber-600" />
                  <span className="hidden sm:inline text-amber-900 font-bold">Daylight</span>
                </div>
              ) : (
                <div className="flex items-center gap-1">
                  <Moon className="w-4 h-4 text-indigo-400" />
                  <span className="hidden sm:inline text-slate-300 font-bold">Midnight</span>
                </div>
              )}
            ...
            {/* Theme Dropdown Menu */}
            {showThemeDropdown && (
              <div className="absolute right-0 mt-2 w-56 rounded-2xl bg-slate-900/95 border border-slate-700/80 shadow-2xl backdrop-blur-xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                        ...
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <div className="text-xs font-bold leading-tight flex items-center gap-1">
                              ...
                              {opt.id === 'golden' && (
                                <span className="text-[9px] bg-amber-500/30 text-amber-300 px-1 rounded font-black border border-amber-500/40">
                                  HOT
                                </span>
                              )}
                            </div>
          ...
          <div
            ...
            className="flex items-center gap-2 bg-slate-800/90 hover:bg-slate-800 border border-slate-700/80 hover:border-amber-500/50 rounded-full px-3 py-1.5 cursor-pointer transition shadow-sm group"
          >
            <div className="flex items-center gap-1.5">
          ...
          <div
            ...
            className="flex items-center gap-2 border-l border-slate-800 pl-2 sm:pl-3 cursor-pointer hover:opacity-90 transition group"
          ...
          >
            <div className="relative">
              <img
                ...
                className="w-8 h-8 rounded-full border-2 border-amber-400/80 object-cover group-hover:scale-105 transition-transform"
              />
              <span className="absolute -bottom-1 -right-1 bg-amber-500 text-slate-950 text-[9px] font-black px-1 rounded-full border border-slate-950 flex items-center gap-0.5">
                <Award className="w-2.5 h-2.5" /> {isLoggedIn ? `L${user.vipLevel}` : 'none'}
              </span>
            </div>
